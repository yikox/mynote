...
# 带信号的 put：数据传输完成后自动通知对方
tdl.putmem_signal_nbi(dest, src, bytes, sig_addr, signal, sig_op, pe)

# 信号操作
tdl.signal_op(sig_addr, signal, sig_op, pe)         # 向远程写信号
tdl.signal_wait_until(sig_addr, cmp_, cmp_val)      # 等待本地信号
...
论文中提到但当前版本未开放：自动 tiling 的 `all_gather`、`reduce_scatter`、`all_to_all` 等集合操作。目前需用低层 put/get 手动拼装。
...
```
初始状态（4 GPU）：
  GPU₀ 持有: Q₀, K₀, V₀    GPU₁ 持有: Q₁, K₁, V₁
  GPU₂ 持有: Q₂, K₂, V₂    GPU₃ 持有: Q₃, K₃, V₃

Step 0（本地）：每卡计算 attention(Qᵢ, Kᵢ, Vᵢ)，异步发送 Kᵢ, Vᵢ 给邻居
Step 1..N-1：收到邻居 KV → 计算 attention → rescale 合并 → 转发 KV
最终：每个 GPUᵢ 拥有完整 Oᵢ = attention(Qᵢ, 所有 KV)
```

### 4.3 Online Rescaling（核心公式）
...
### 4.4 Zigzag Ring Attention（因果 mask 优化）

重新排列 chunk 顺序，提升 causal ratio，减少无效计算。
...
### 5.2 Tile 级 Overlap 方案（双 kernel 架构）
...
# 计算 kernel（主 stream）：每个 tile 用 wait/consume_token 绑定通信完成
@triton.jit
def compute_kernel(...):
    for round in range(world_size):
...
这是 **TileLink** 论文中的 tile-centric primitive 方法——通信与计算解耦为独立 tile 流，通过 signal token 建立依赖图，编译器在 LLVM 层面做流水线调度。
...
| **GEMM + ReduceScatter** | 分布式矩阵乘+规约 |
| **Low-latency AllToAll** | 137μs vs DeepEP 182μs（32 H800） |

### 6.2 ring-flash-attention（zhuzilin）

基于 FlashAttention kernel + NCCL P2P 的 Ring Attention 最佳参考实现。8×H800 上 zigzag ring attention 达到理论 FA 上限的 85%（fwd）/ 90%（fwd+bwd）。
...
6. 通信 kernel 通过 `num_warps` 控制 SM 占用，避免与计算 kernel 争抢
...
| **USP / YunChang** | [GitHub](https://github.com/feifeibear/long-context-attention) | Ulysses + Ring hybrid |
| **FlashAttention-3** | [arxiv](https://arxiv.org/abs/2407.08608) | Hopper FA，Triton 参考 |
