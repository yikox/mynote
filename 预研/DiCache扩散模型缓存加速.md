...
为方便阅读，先统一后续章节用到的核心符号：

| 符号              | 含义                                                                  | 维度 / 备注              |
| --------------- | ------------------------------------------------------------------- | -------------------- |
| $t$             | 当前去噪时间步的索引                                                          | $\in \{1, ..., T\}$   |
| $T$             | 总采样步数（论文中视频模型用 50 步）                                                 | 标量                   |
| $x_t$           | **时刻 t 的模型输入**（带噪 latent）                                             | 与 $y_t$ 同维度            |
| $x_{t+1}$       | **时刻 t+1 的模型输入**                                                     | 与 $x_t$ 同维度            |
| $y_t$           | **完整 DiT 模型在时刻 t 的最终输出**（velocity prediction）                      | 与 $x_t$ 同维度            |
| $y_t^m$         | **DiT 前 m 层（probe）在时刻 t 的中间输出**（浅层特征）                              | 与 $x_t$ 同维度，m≪M     |
| $M$             | DiT 的总层数（如 WAN 2.1 / HunyuanVideo 通常是 30+ 层）                            | 标量                   |
| $m$             | **Probe 深度**（DiCache 默认 1）                                            | $\in \{1, 2, ..., M\}$ |
| $f_t^m$         | **Probe 浅层特征**（同 $y_t^m$，下文中这两个符号通用）                                  | 与 $x_t$ 同维度            |
| $r_t$           | **完整模型残差** = $y_t - x_t$                                               | 表示 latent→velocity 方向 |
| $r_t^m$         | **浅层 probe 残差** = $y_t^m - x_t$                                        | 与 $r_t$ 同维度            |
| $\|\cdot\|_1$   | L1 范数（向量元素绝对值之和）                                                      | 标量                   |
| $\|\cdot\|$     | L2 范数（欧氏距离，个别公式用）                                                     | 标量                   |
| $\delta$        | **缓存决策阈值**（浅层差异低于此值时复用缓存）                                             | 超参                    |
| $k$             | **历史缓存步数**（参与加权组合的过往输出个数）                                              | 超参，2~4                |
| $\alpha$        | **softmax 温度系数**（越大越尖锐，越小越平滑）                                          | 超参，默认 1.0             |
| $d(\cdot,\cdot)$ | **probe 特征空间距离**（欧氏或余弦距离）                                              | 标量                   |
| $w_i(t)$        | **第 i 个历史缓存步的组合权重**（由 probe 距离 softmax 得到）                            | $\in [0, 1]$，和为 1      |
| $v_t$           | **模型最终用于 scheduler.step 的输出**（与 $y_t$ 等价，下文用 $v$ 表示 velocity）          | 与 $x_t$ 同维度            |
| $v_{t_i}$       | **历史时间步 $t_i$ 的 velocity 输出**（缓存项）                                    | 与 $v_t$ 同维度            |
...
**形式化**：用**归一化 L1 距离** $L1_{\text{rel}}(\cdot, \cdot)$ 衡量两个向量之间的相对变化幅度：
...
* $a, b$：**任意两个同维向量**（如 $x_t$ 与 $x_{t+1}$，或 $y_t^m$ 与 $y_{t+1}^m$）
* $\|a - b\|_1$：$a$ 与 $b$ 的 L1 距离，反映**绝对变化量**
* $\|a\|_1$：归一化分母，反映 $a$ 的**量级**
* 整个比值 = **相对变化率**（无量纲，跨 timestep 可比）

**论文 Fig. 3 比较三个量的变化模式**：

| 指标                              | 含义（$a$ / $b$ 是什么）              | 表现                | 与输出差异相关？     |
| ------------------------------- | -------------------------------- | ----------------- | ------------ |
| $L1_{rel}(y_t, y_{t+1})$        | **输出差异**（$a=y_t$, $b=y_{t+1}$）   | 方差大、**样本特定**（图 a） | —            |
| $L1_{rel}(x_t, x_{t+1})$        | **输入差异**（$a=x_t$, $b=x_{t+1}$）   | 随时间步**单调增加**（图 b） | ❌ 几乎不相关     |
| $L1_{rel}(y_t^m, y_{t+1}^m)$    | **浅层特征差异**（$a=y_t^m$, $b=y_{t+1}^m$） | **与输出差异模式高度吻合**（图 c） | ✅ 强相关        |

#### 2.1.2 量化：Spearman 相关系数

论文 Fig. 3(d) 给出**浅层 ↔ 深层**差异的 Spearman 相关系数（即 $L1_{rel}(y_t^m, y_{t+1}^m)$ 与 $L1_{rel}(y_t, y_{t+1})$ 的秩相关系数）：
...
$$
r_t \;=\; y_t - x_t, \qquad r_t^m \;=\; y_t^m - x_t
$$

其中：

* $r_t$：**整个 DiT（M 层）的残差方向**（latent $x_t$ 出发、到达 velocity $y_t$ 的向量）
* $r_t^m$：**仅前 m 层的残差方向**（latent $x_t$ 出发、经过 m 层后到达 $y_t^m$ 的向量）
* $r_t$ 与 $r_t^m$ 同维度，**方向对齐 / 幅度不同**（详见 2.2.3）

> 📝 注意：$r_t^m$ 用的是 $y_t^m - x_t$（不是 $y_t^m - x_t^m$），因为浅层 probe 也以原始 latent $x_t$ 为输入起点，对比 $x_t$ 更有意义。论文 Fig. 4(a) 用的就是这一定义。
...
设 probe 在时刻 t 输出浅层特征 $f_t^m$（同 $y_t^m$，下文中这两个符号通用），**归一化特征差异**：
...
* $f_t^m$：**当前 t 时刻的浅层 probe 特征**（同 $y_t^m$）
* $f_{t-1}^m$：**上一 t-1 时刻的浅层 probe 特征**
* $\| f_t^m - f_{t-1}^m \|$：**两时刻浅层特征的 L2 距离**（绝对变化量）
* $\| f_{t-1}^m \|$：归一化分母（上一时刻特征的量级）
* 整个比值 = **相对变化率**（与 §2.1.1 的 $L1_{rel}$ 等价，只是这里用 L2）
...
利用"层间轨迹相似"（观察 2），probe 的浅层特征轨迹"导航"深层的近似。当前 velocity 用**历史 k 步 velocity 的加权和**近似：
...
* $v_t$：**当前 t 时刻我们想得到的 velocity 输出**（DiT 应跑出来的真值）
* $v_{t_i}$：**历史 $t_1, t_2, ..., t_k$ 时刻实际跑过 DiT 缓存下来的 velocity**（$t_1$ 是最近一次跑过主干的时刻）
* $w_i(t)$：**第 i 个历史步的组合权重**（满足 $\sum_i w_i = 1$）
* $k$：**历史缓存步数**（超参，2~4）

**权重**由 probe 特征距离经 softmax 得到（距离越近，权重越大）：

$$
w_i(t) \;=\; \frac{\exp\!\big(-\alpha \cdot d(f_t^m, f_{t_i}^m)\big)}{\sum_{j=1}^{k} \exp\!\big(-\alpha \cdot d(f_t^m, f_{t_j}^m)\big)}
$$

其中：

* $f_t^m$：**当前 t 时刻的 probe 浅层特征**
* $f_{t_i}^m$：**历史 $t_i$ 时刻的 probe 浅层特征**（与 $v_{t_i}$ 一起缓存下来）
* $d(\cdot, \cdot)$：**probe 特征空间距离**（欧氏或余弦距离）
* $\alpha$：**softmax 温度系数**（默认 1.0；越大权重越尖锐——距离近的占绝对优势；越小越平滑——近似均匀加权）
* $i$ / $j$：历史缓存步的索引（$i, j \in \{1, ..., k\}$）
