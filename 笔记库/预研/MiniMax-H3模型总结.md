...
各模态被转换为对应 token/latent 后，经过输入整理步骤组成统一的 packed multimodal sequence。它本身不是独立的神经网络模块，而是把不同模态的表示整理成 Transformer 能处理的统一序列。
...
- **文本 hidden states**：来自 H3-Encoder 的文本语义表示。
- **视觉语义 hidden states**：图片/视频经过 H3-Encoder 后得到的第 50 层 hidden states，可视为语义参考序列。
- **视觉 VAE latents**：图片/视频经过 VisualVAE 后得到的视觉 latent，并在进入 Transformer 前进行 patchify。
- **音频 VAE latents**：音频经过 AudioVAE 后得到的音频 latent 序列。
- **`[L, 5376]`**：`L` 是所有模态 token 的总长度，`5376` 是 H3-Omni-Transformer 的共同 hidden size。

生成时还会加入待预测的视频/音频 latent 槽位；上图只保留主要的条件序列。packed sequence 主要负责投影、布局、拼接和位置元数据构造，真正的跨模态交互发生在后续共享的 Attention 和 FFN 中。
