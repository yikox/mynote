...
已选择 `llama.cpp + UD-Q8_K_XL GGUF + 8 卡 layer split` 为主线，让跨 NUMA 的关键通信主要集中在层边界。官方 0731 模型为 304B、混合 BF16/FP8 权重；Unsloth 将 162 GB 的 `UD-Q8_K_XL` 标注为 full-precision lossless 档。该 Q8 只比 Q4 大约 6.3 GiB，显存理论余量仍足以从 4K context 开始验证，因此当前不为节省少量空间先牺牲非 expert 权重精度；TP8/EP 只保留通信基准结论，不再作为首轮部署路线。
...
- 基线运行时显式关闭 CUDA Graphs；另以同一提交、同一编译参数构建了 `runtime_graphs/` 候选，已通过 `ldd` 与 8 卡枚举，但不会在正确性和严格输出一致性对照前替换基线。
...
- 服务脚本 `/root/deepseek-v4-flash-0731/runtime/ds4-server.sh` 默认绑定 `127.0.0.1:8000`，会检查 5 个分片大小、端口和 PID；模型不完整时已验证会安全拒绝启动。
- 脚本支持参数化 runtime、context、parallel、batch、ubatch 与 continuous batching，默认仍为 4K、单并发、batch 128、ubatch 32、关闭连续批处理。
- 已准备 2K/4K 固定 seed 上下文一致性脚本，以及 OpenAI API 流式/非流式压测脚本；后者输出成功率、TTFT、端到端 p50/p95、prefill、decode 与聚合 tokens/s。
- 吞吐 A/B 必须保持每个 slot 4096 context、Q8 权重和 F16 K/V 不变，比较并发 1/2/4、连续批处理、batch/ubatch、CUDA Graphs 与 NUMA；只有输出一致且稳定的候选才能成为最终配置。

## 进行中

1. 完成剩余 4 个 Q8 分片下载并逐文件校验固定 Hugging Face SHA256；当前仅此步骤受约 8–10 小时网络耗时约束，四个 aria2 进程均正常、磁盘余量约 1.2 TB。
2. 先执行 2K/4K 固定输出一致性，再以 4K、单并发、关闭 DSpark、F16 KV cache 启动 8 卡 layer split 最小闭环。
3. 验证官方 0731 模板、OpenAI `/v1/models`、流式/非流式 chat completions 和 reasoning 行为，并完成 100 请求正确性闸门。
4. 通过上述硬闸门后，再控制变量优化 batch、ubatch、NUMA、CUDA Graphs 与并发，完成稳定性测试后固化最佳配置。
