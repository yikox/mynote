...
- [ ] **Step 2: 运行单测确认新增要求成立，并记录 demo 仍未消费 helper**
...
- [ ] **Step 3: 修改 Stage2 demo 的 import、初始化和配置装配**
...
- [ ] **Step 5: 只提交 Stage2 demo 与新增的单卡测试**
...
def _video_ssim(reference: np.ndarray, candidate: np.ndarray) -> float:
    ...
    lo = min(float(reference.min()), float(candidate.min()))
    hi = max(float(reference.max()), float(candidate.max()))
...
def compare_dumps(reference_prefix: str, candidate_prefix: str) -> dict[str, float]:
    ...
    return {
        "video_ssim": _video_ssim(ref_video, cand_video),
...
def validate_matrix(
    ...
    control = compare_dumps(reference_prefix, control_prefix)
    report = {"control": control, "candidates": {}}
    failures = []
    for name, prefix in candidates:
        metrics = compare_dumps(reference_prefix, prefix)
        report["candidates"][name] = metrics
        if metrics["video_ssim"] < control["video_ssim"] - 0.02:
            failures.append(
                f"{name}.video_ssim={metrics['video_ssim']:.6f} < "
                f"control-0.02={control['video_ssim'] - 0.02:.6f}"
            )
...
Expected: `Ran 3 tests` 与 `OK`；py_compile 成功。
...
- [ ] **Step 2: 同步 OptKit V2 和 Stage2 文件**
...
- [ ] **Step 2: 跑 2 卡 Ulysses 控制组 `u2/r1`**
...
- [ ] **Step 4: 跑 4 卡 Ring + Ulysses `u2/r2`**
...
- [ ] **Step 8: 开 compile 跑三种多卡 production 配置**
...
如果任何硬门槛未通过，以上状态保持 implementing/pending，只记录已确认的阻塞与证据，不得标记完成。
