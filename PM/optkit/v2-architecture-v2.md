# optkit_v2 架构讲解（v2 · 分享文档）

> 面向**新接入贡献者**的代码导读。讲清三件事：**支持哪些优化方法及其限制**、**Context 的设计**、**已实现的模型 pipeline**，以及**契约层面的纪律**。
> 代码路径：`optkit_v2/`。
>
> v2 在 v1 基础上按「TL;DR + 详解」重组，新增 HOOK_ORDER 总表、能力互斥矩阵、不支持清单、报错排查；侧重 **Context 契约**（§2 占 ~30%）。
...
- [0. 一分钟总览](#0-一分钟总览)
- [1. 优化方法与限制](#1-优化方法与限制)
- [2. Context 契约（核心）](#2-context-契约核心)
- [3. 已支持的模型 pipeline](#3-已支持的模型-pipeline)
- [4. 新增 pipeline 路径](#4-新增-pipeline-路径)
- [5. 常见报错与排查](#5-常见报错与排查)
...
```mermaid
flowchart TB
    User["用户<br/>OptKitConfig(specs)"]:::cfg
    Cfg["config/specs.py<br/>描述意图，无逻辑"]:::cfg
    Apply["apply.py<br/>spec → component"]:::apply
    Comp["OptComponent<br/>能力插件<br/>sage/quant/compile/parallel/cache/regione"]:::comp
    Ctx["OptKitContext（冻结契约）<br/>core/context.py<br/>分发原语 + 生命周期 + 引用槽"]:::ctx
    Warp["warp（模型侧）<br/>warps/*.py<br/>替换 transformer.forward + attn processor"]:::warp

    User --> Cfg --> Apply
    Apply --> Comp
    Comp <-->|register / trigger hooks| Ctx
    Ctx <-->|trigger 发 hook| Warp

    classDef cfg fill:#fef3c7,stroke:#b45309
    classDef comp fill:#dbeafe,stroke:#1d4ed8
    classDef ctx fill:#fce7f3,stroke:#be185d,stroke-width:2px
    classDef warp fill:#dcfce7,stroke:#15803d
    classDef apply fill:#ede9fe,stroke:#6d28d9
```

**核心契约**：`OptKitContext` 是 warp ↔ component、component ↔ component 之间的稳定边界——**修改本类 = 契约变更，需评审**（详见 §2）。
...
sequenceDiagram
    ...
    participant C as OptKitContext
    participant K as OptComponent(s)
    participant W as warp_fn

    U->>A: apply_warp(pipe, cfg)
    A->>C: 校验 pipe.transformer._optkit_ctx 未设
    A->>A: resolve_warp(pipe) 按 MRO 类名解析
    A->>K: 逐个 spec_to_component → attach(ctx)
    Note over K: register handlers（顺序无关）
    A->>W: warp_fn(pipe, ctx)
    W->>W: 替换 transformer.forward + attn processor
    A->>C: trigger("on_pipe_ready", pipe)
    C->>K: 按 HOOK_ORDER fold：quant → compile → regione
    Note over K: 量化改权重 / 编图 / 换 scheduler
    A-->>U: ctx
```
...
| 能力 | 装在哪 | 适合 | 不适合 |
| --- | --- | --- | --- |
| SageAttention | `components/sage/` | 所有 attn 后端加速 | 需装 `sageattention` |
| Quant (FP8/INT8) | `components/quant/` | 缩常驻显存 | 不能 + cpu-offload |
| torch.compile | `components/compile/` | 稳态加速 | 必须跑两遍；变长序列 partial step 须 `dynamic=True` |
| Parallel (Ulysses/Ring/USP) | `components/parallel/` | 长序列多卡切分 | Qwen t2i + ulysses4 + true CFG 会 NaN（见 §3.2） |
| DiCache / MagCache | `components/cache/` | 步级跳块复用 | 阈值须逐任务标定 |
| RegionE | `components/cache/regione/` | 序列级参考编辑加速 | 仅 Edit / Kontext / FLUX2-Klein；不支持 Ring |
...
> ⚠️ 需安装 `sageattention`，否则 `attach` 直接 `RuntimeError`。
...
> ⚠️ 量化后 transformer 不做 cpu-offload（torchao 量化张量与 accelerate 跨设备搬运不兼容）。

- **做什么**：基于 torchao 对 transformer 权重做 FP8/INT8 量化，缩小常驻显存。`dtype` 支持 `float8/fp8/int8` 及各自 `_weight_only` 变体。
- **怎么接**：是 pipe 变换（改权重），注册到 `on_pipe_ready`；按 `HOOK_ORDER["on_pipe_ready"] = (QUANT, COMPILE, REGIONE)`，**量化早于 compile**（编译已量化的权重）。
...
- **限制 / 注意**：
  - 显存不足靠量化缩到常驻，而非 offload。

### 1.3 torch.compile（`components/compile/`）—— 区域编译

> ⚠️ **必须跑两遍**：第一遍含编译开销，计时/对比结论取第二遍稳态。
...
> ⚠️ 阈值须逐任务标定；标定过大会出马赛克。t2i 类对基线 SSIM 仅弱参考（无锚定，漂移地板 ~0.5），质量结论须看图 + 对 sfc 算 SSIM。
...
> ⚠️ **不支持 Ring**；**+ Ulysses 必须 `ulysses_anything=True`** 且 `ring_anything` 被强制关；**+ compile 强制 `dynamic=True`**。
...
```mermaid
flowchart LR
    subgraph Pipeline["① pipeline 层<br/>on_denoise_step_pre / post"]
        A1["把 noise 按 edited_ids 切成 edited 子集<br/>rotary 同步切"] --> A2["ids_scatter 还原"]
    end
    subgraph Attn["② attn 层<br/>on_backend_enter<br/>(parallel 之后)"]
        B1["store 步：存 post-rotary full K/V"] --> B2["partial 步：从 cache 拼 full K/V"]
    end
...
| Hook 注册点 | 参与者顺序 | 用途 | 语义 |
| --- | --- | --- | --- |
| `on_pipe_ready` | `(QUANT, COMPILE, REGIONE)` | 延迟的 pipe 变换 | 量化改权重 → 编图 → 换 scheduler |
| `replace_dispatch_attention_fn` | `(SAGE, PARALLEL)` | attn 后端 + CP 内核 | sage replace → ring decorate |
| `on_backend_enter` | `(PARALLEL, REGIONE)` | attention 外层变换 | Ulysses scatter heads → RegionE KV 组装 |
| `on_denoise_step_pre` | `(REGIONE, PARALLEL)` | 步级序列切分 | regione 切 edited → CP 切 1/cp |
| `on_denoise_step_post` | `(PARALLEL, REGIONE)` | 步级序列还原 | CP gather unwind → regione ids_scatter |
| `should_skip_block` | `(CACHE, …)` | 跳块复用残差 | 各 cache ABC 顺序 fold |

> 未在此表登记的 `(hook, component)` 在 `register` 时直接报错。新增 hook / 新参与 component 必须先改 `core/order.py`。

### 2.3 能力互斥 / 协同矩阵

| | Sage | Quant | Compile | Ulysses | Ring | DiCache | MagCache | RegionE |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **Sage** | — | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 包裹 | ✅ 正交 | ✅ 正交 | ✅ 正交 |
| **Quant** | ✅ 正交 | — | ✅ 早于编图 | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 |
| **Compile** | ✅ 正交 | ✅ 早于 | — | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 | ⚠️ 强制 `dynamic=True` |
| **Ulysses** | ✅ 正交 | ✅ 正交 | ✅ 正交 | — | ✅ 组合=USP | ✅ 正交 | ✅ 正交 | ⚠️ 必须 `ulysses_anything=True` |
| **Ring** | ✅ 包裹 | ✅ 正交 | ✅ 正交 | ✅ 组合=USP | — | ✅ 正交 | ✅ 正交 | ❌ config 报错 |
| **DiCache** | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 | — | ❌ cache 三选一 | ❌ cache 三选一 |
| **MagCache** | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 | ✅ 正交 | ❌ cache 三选一 | — | ❌ cache 三选一 |
| **RegionE** | ✅ 正交 | ✅ 正交 | ⚠️ `dynamic` | ⚠️ `anything` | ❌ | ❌ cache 三选一 | ❌ cache 三选一 | — |

> 包裹 = 后者套前者（ring 内层走 sage kernel）；正交 = 不耦合；互斥 = config 层直接报错。

### 2.4 延迟 pipe 变换到 `on_pipe_ready`
...
sequenceDiagram
    ...
    participant K as Compile
    participant R as RegionE

    Note over A: warp_fn 完成，forward 已替换
    A->>C: trigger("on_pipe_ready", pipe)
    C->>Q: 1. torchao 改权重
    C->>K: 2. block.compile() 编图
    C->>R: 3. 换 RegionEFlowMatchScheduler
```
...
| 模型族 | Pipeline 类名 | 文件 | 任务 | 参数量 | 单卡显存下限 | RegionE 适用 | 实测加速 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Qwen-Image | `QwenImagePipeline` | `pipeline_qwenimage.py` | t2i 文生图 | 20B | 32G（量化后） | 否（无参考图） | — |
| Qwen-Image-Edit | `QwenImageEditPipeline` | `pipeline_qwenimage_edit.py` | 单图编辑 | 20B | 32G（量化后） | 是 | u4+regione 待补 |
...
| Wan-I2V | `WanImageToVideoPipeline` | `pipeline_wan_i2v.py` | 图生视频 | 14B / A14B | — | 否 | — |
| Wan-VACE | `WanVACEPipeline` | `pipeline_wan_vace.py` | VACE 控制生成 | 14B / A14B | — | 否 | — |

> 加速比 = 优化栈相对 baseline 的 latency 倍率；空格 = 暂无数据或不适配。

### 3.2 成熟度分层

- **🟢 已验证**（4×RTX5090 全矩阵：baseline / sage+fp8+compile / +dicache / ulysses4，稳态零重编）：Qwen / FLUX 系（t2i / 编辑 / Fill / Inpaint / Kontext / FLUX2-Klein）。
- **🟡 待验证**：Wan t2v/ti2v/VACE 部分路径标注「盲写，待真实权重验证」（M4 收口项）。
- **🔴 已知坑**：
  - **Qwen t2i + ulysses4 + true CFG → latent NaN 全黑图**，触发条件为 CFG 两分支 txt 长度不一致；规避：负 prompt embeds pad 到同长，或 `true_cfg_scale=1.0`。Qwen edit 系不受影响。
  - **Qwen ring 数值漂移**：ring4=0.70、u2r2=0.58 vs ulysses4=0.88（SSIM 偏低，排查中）。
  - **USP 路径**：需真机 2×2 数值对比验证后方可信（代码标注 ⚠️）。

### 3.3 不被支持的 pipeline 类型（显式拦截）

为避免新同学乱试，下列类型在 `apply_warp` 阶段会**直接报错拦截**，不属于「能跑但效果差」：

| 类型 | 原因 |
| --- | --- |
| 纯 t2i 无参考图 | RegionE 不适用（无 partition 依据） |
| Fill 通道级条件 | FLUX.1-Fill：条件以通道维注入，RegionE 的 token 级 partition 不适用 |
| Inpaint mask 混合 | FLUX.1-Inpaint：mask 混合与 RegionE 冻结未编辑区域语义冲突 |
| Wan 视频类 t2v / i2v / VACE | 走 cross-mode 透传，RegionE 全程不接管；且 Wan 路径部分盲写 |
...
| 现象 | 触发条件 | 根因 | 排查方向 |
| --- | --- | --- | --- |
| `attach` 时 `RuntimeError: sageattention not installed` | 启用 `SageSpec()` 但没装包 | sage import 失败 | `pip install sageattention`；不需要可关 sage |
| `register` 抛 `KeyError/ValueError` | 新 hook 或新 component 未在 `HOOK_ORDER` 登记 | 顺序事实源缺失 | 改 `core/order.py` 加 `(注册点, 顺序元组)` |
| `__post_init__` 报错「regione + ring 互斥」 | 同时开 RegionE 和 Ring | ring 的 seq-partition K/V 与 RegionE 全局 cache 算法不兼容 | 关 ring：`ulysses_degree>1, ring_degree=1` |
| `__post_init__` 报错「regione 必须 ulysses_anything=True」 | regione + ulysses 且 `ulysses_anything=False` | Qwen txt/img 序列不保证整除 cp_size | `ParallelSpec(ulysses_degree=N, ulysses_anything=True)` |
| regione + compile 报 `dynamic=True` 警告 | regione + compile 且未开 dynamic | partial step 变长序列，dynamo 反复重编每步几分钟 | config 自动覆盖；如手动设 false 会 warn |
| 推理中途缓存窗口清空 | DiCache/MagCache + true CFG 但 `cfg_enable=False` | cnt 到 num_steps 中途 reset | 设 `cfg_enable=True` |
| 真黑图 / latent NaN | Qwen t2i + ulysses4 + true CFG + 两分支 txt 长度不同 | txt pad 不一致导致 scatter/gather 错位 | 负 prompt embeds pad 到同长，或 `true_cfg_scale=1.0` |
| Ring 路径下 Qwen SSIM 偏低 | ring + Qwen（任意 cfg） | Qwen 专属 RoPE/attention 与 ring 在线 softmax 数值漂移 | 暂用 ulysses；flux-kontext ring 正常 |
| t2i 任务 SSIM 0.5 左右 | 用 SSIM 直接评 t2i baseline | t2i 无锚定，漂移地板 ~0.5 | 看图主观评 + 对 sfc 算 SSIM |
| compile warmup 内多次重编 | 启用 compile | fp8 权重类型 guard + CFG 两种序列长 | 必须跑两遍，结论取 run2 稳态 |
| USP 数值异常 | `ulysses>1 and ring>1` | 2D mesh 路径未充分验证 | 暂用纯 ulysses 或纯 ring；待 2×2 真机回归 |
| 单卡 OOM（Qwen 20B RegionE 1024²） | 大模型 + regione 单卡 | RegionE KV cache + edited query 全量 attend | 启用 ulysses4 序列切分 |
| Quant 后搬设备报错 | transformer + cpu-offload | torchao 量化张量与 accelerate 跨设备搬运不兼容 | 关 cpu-offload，靠量化缩到常驻 |

---

> **下一步**：本版本重在结构与展示。后续若实测加速比 / Wan 验证 / USP 验证 / Qwen ring 排查有更新，请同步 §3.1、§3.2 与 §5。
