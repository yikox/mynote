...
### 第 1 步：改版本号并提交（命令）
以 `v0.2.0` 为例。**实际共 5 个文件**带项目版本号，全部要改成一致：
`package.json`、`package-lock.json`（2 处：根 `version` 和 `packages[""].version`）、`src-tauri/tauri.conf.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock`（仅 `[[package]] name = "plexus"` 那一处）。
```bash
npm version 0.2.0 --no-git-tag-version          # 改 package.json + package-lock.json
# 手动改 src-tauri/tauri.conf.json、Cargo.toml、Cargo.lock 的 plexus 版本
git add package.json package-lock.json src-tauri/tauri.conf.json src-tauri/Cargo.toml src-tauri/Cargo.lock
git commit -m "release: v0.2.0"
git push origin main
```
> ⚠️ **Cargo.lock 陷阱**：盲目 `sed 's/0.4.10/0.4.11/'` 会误伤同号的第三方 crate。实例：v0.4.11 时 Cargo.lock 既有 `plexus`(line 2900) 也有 `erased-serde`(line 963) 都是 `version = "0.4.10"`，只能按行号精确改 plexus 那一处。改后先 `grep -rn '0.4.11' …` + `sed -n '963p'` 确认第三方未动。
> 漏改任一文件，或 tauri.conf.json 与 tag 不一致，会在「Validate tag」步骤失败。
