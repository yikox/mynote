...
- 2026-07-04（BUG-20260704-editor-input-scroll-jump，L1）: 修复 rich 编辑器「输入时视口被动往下跳、光标被甩到页面下方」。根因是 `ModuleTextarea` 的命令式 DOM 操作在滚动容器 `.markdown-editor__scroll` 里引发被动滚动:①自适应高度先把 `height` 置 `'auto'` 再读 `scrollHeight`,这次重排会钳制容器 `scrollTop` 且恢复 `height` 不还原它(每次按键都发生,是主因);②`focus()`/`setSelectionRange()` 未用 `preventScroll`,聚焦/光标定位会把焦点元素滚入视野(结构性编辑/点击时)。均为既有代码,非本次 WYSIWYG 重构引入。修复:新增 `withNeutralScroll()` 包装三处 useLayoutEffect(挂载聚焦、caretNonce 重定位、自适应高度),先记录再还原容器 `scrollTop`,并把 `focus()` 改为 `focus({ preventScroll: true })`。验证:新增 3 例回归(preventScroll 聚焦 x2、被动滚动被还原 x1),661/661 测试、`tsc --noEmit`、`npm run build` 均通过。
