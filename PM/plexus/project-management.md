...
| `REQ-20260704-agent-runtime-port` | AI 编排层依赖注入端口（AgentRuntime）：打破 4 个编排文件对 store 单例的穿透，使编排层依赖抽象端口/参数注入、可隔离测试；正式区分 ai-core/ai-orchestration 子层 + guard 测试 | `ai-agent-loop` | `ai-context-tools`, `ai-chat-surface`, `ui-state-stores`(约束级) | L3 | accepted · implemented-in-branch, pending-merge | P2 | **待用户合并** `worktree-agent-runtime-port`（8 提交 55c69b8..3079189，649/649 绿、build ok、guard 3/3）→ 合并后落基线(main-design 子层+materializeImages 归 ai-agent-loop)+图+PM complete。设计/ADR/计划见 Design Index |
...
- 2026-07-05 done（BUG-20260705-chat-link-fullpage-nav，L1，primary `ai-chat-surface`）: 修复 AI 聊天消息里点击相对链接（如 `[x](./README.md)`）导致 webview 整页跳转到本地文件、SPA 被原始乱码内容替换（丢失应用状态）。根因：`MessageItem.tsx` 的 `ReactMarkdown` 只覆写了 `pre`，未拦截 `<a>`，且全应用无全局导航守卫——编辑器侧早有 `handleLinkActivate`+`preventDefault` 守卫，聊天侧从未接线，故 webview 执行原生导航（无 charset→乱码）。修复：新增 `MarkdownAnchor` 组件（`preventDefault` + 复用 `resolveLink`：external→`openUrl`、internal→`openNote`+`revealPath`、相对路径按 workspace 根解析），接线到两处 `ReactMarkdown` 的 `components.a`。改动 `src/components/AIChat/MessageItem.tsx`、`MessageItem.test.tsx`（+2 回归：相对链接拦截并应用内打开、外链走系统打开器；jsdom「navigation to another Document」由 1→0）。验证：670/670 测试、`tsc --noEmit`、`npm run build` 均通过。**未提交**（主工作区尚有编辑器 WIP，避免混提交）。遗留（低）：`MessageItem` 现 import `Editor/linkClickExtension` 形成 `ai-chat-surface→markdown-module-engine` 跨模块依赖（`resolveLink` 是纯通用工具，宜后续移入共享位置）；更彻底的防御是加一层全局导航守卫（任何未拦截 `<a>` 都不得整页跳转），作为可选 backlog。
