...
- 2026-07-05 done（BUG-20260705-link-fullpage-nav，L1，primary `app-shell-layout`，impacted `ai-chat-surface`）: 修复点击相对链接（如 `[x](./README.md)`）导致 webview 整页跳转到本地文件、SPA 被原始乱码内容替换（丢失应用状态）。根因（真正的）：**全应用无全局导航守卫**——只要有任一处 `<a>` 未被组件自行拦截，webview 就执行原生导航（无 charset→乱码）。首次只补了聊天侧（`MessageItem` 的 `ReactMarkdown` 未覆写 `a`）不够；用户复现仍在，说明泄漏面不止一处，逐一堵易漏判。故加**根因层修复**：`useLinkNavigationGuard`（挂在 `AppShell`）在 document 冒泡阶段兜底所有未被 `preventDefault` 的 `<a>` 点击——尊重组件已有处理（编辑器按当前笔记路径解析，`defaultPrevented` 即跳过），只对没人管的链接按 workspace 根解析：external→`openUrl`、internal→`openNote`+`revealPath`、锚点/未知→仅阻止跳转。两层防御（组件层 + 全局兜底）。改动：新增 `src/components/Layout/useLinkNavigationGuard.ts(.test.tsx)`、`AppShell.tsx` 挂载；聊天侧 `MessageItem.tsx(.test.tsx)` 的 `MarkdownAnchor`。验证：673/673 测试、`tsc`、`build` 通过；guard 3 例 + 聊天 2 例回归。**未提交**（主工作区有编辑器 WIP，避免混提交）；用户需重启 dev/重载 app 生效。遗留（低）：`resolveLink` 被 chat/shell/editor 三处 import（现居 `Editor/linkClickExtension`），宜后续移入共享位置。
