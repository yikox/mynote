...
| `REQ-20260705-ai-git-readonly-tools` | AI 只读 Git 检视工具集：`git_log`/`read_note_at`/`git_diff`/`git_status`/`git_show_commit`（全只读、git2、工作区内、无 shell/透传）；恢复靠组合 read_note_at+update_note，不加任何 git 写工具 | `ai-context-tools` | `git-local-history`, `tauri-service-adapters`, `ai-agent-loop`, AIConfig | L2 | accepted（modular-autopilot 执行中） | P2 | 设计 accepted+reviewed：`architecture/modules/ai-context-tools/changes/2026-07-05-ai-git-readonly-tools.md`（全量 spec 仓库 `docs/superpowers/specs/2026-07-05-ai-git-readonly-tools-design.md` dc3c605）。autopilot：writing-plans → HEAD 基 worktree SDD → pending-merge |
