...
| 2026-06-24 | 代码检查确认 `DuckDbParquetBarStore` 已包含 `load_many(symbols, start, end)`，PM 里原“批量查询 API”下一步不应再当作完全未实现需求处理。 |
| 2026-06-24 | `LongOnlyBacktester` 当前仅包含手续费、滑点和 100 股手数逻辑，尚未实现 T+1、涨跌停、停牌等 A 股交易规则。 |
...
| 2026-06-24 | Project Memory 规则补充需求待办、设计文档和架构文档路径 | 支持 `pm-record-requirement` -> `pm-design-requirement` -> `pm-document-architecture` 的 PM lifecycle 试跑 |
